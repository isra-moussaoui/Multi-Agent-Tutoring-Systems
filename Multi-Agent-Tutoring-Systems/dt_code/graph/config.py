"""Per-role LLM config passed via LangGraph RunnableConfig['configurable']."""
...
def role_from_configurable(configurable: dict[str, Any] | None, role: str) -> RoleConfig:
    """Read a RoleConfig for `role` from RunnableConfig['configurable']."""
    ...
    return RoleConfig(
        ...
        sleep_s=float(raw.get("sleep_s", defaults.sleep_s)),
    )
